...
MED_acoustic_projects <- c("ResMed","CONNECT-MED")#projects we want to get the info from

# acoustic deployments

all_deployments <- 
  etn::get_acoustic_deployments(open_only = T) %>%
    dplyr::filter(!is.na(deploy_longitude) & !is.na(deploy_latitude)) %>%
    sf::st_as_sf(., coords = c("deploy_longitude", "deploy_latitude"), crs = 4326)

deployments_MED <- all_deployments %>%
  filter(acoustic_project_code %in% MED_acoustic_projects)
...
current_deployments <- etn::get_acoustic_deployments(deployment_id = deployments_MED$deployment_id) 

# get information on receivers currently deployed
current_receivers <- etn::get_acoustic_receivers(receiver_id = current_deployments$receiver_id)

# get last deployment of each of the receivers currently deployed
last_deployments <- 
  etn::get_acoustic_deployments(receiver_id = current_deployments$receiver_id,
                                open_only = FALSE) %>%
...
This dashboard gives an overview on all open acoustic deployments within the boundaries of the Belgian Part of the North Sea. 


> The deployment data was last updated on: `r Sys.Date()`.


<!-- ```{r valid-asof, include = TRUE} -->

<!-- valueBox(Sys.Date(),  -->
<!--          caption = "The deployment data was last updated on: ", -->
<!--          icon = "fa-calendar", -->
<!--          color = "white") -->
...
```{r testtable, include = FALSE, echo=FALSE}
# for the moment, I only get the datatable() obj to show in Tab 2 & 3 when this code chunk is there...Magic
DT::datatable(all_deployments %>% head())
```
...
deployments_ResMed <- current_deployments %>% 
                     dplyr::filter(acoustic_project_code == "ResMed")

deployments_CONNECTMED <- current_deployments %>% 
                     dplyr::filter(acoustic_project_code == "CONNECT-MED")


icon_anchor_resmed <- leaflet::makeAwesomeIcon(
  icon = "anchor",
  iconColor = 'lightgray',
  markerColor =  ifelse(deployments_ResMed$needs_battery_change == "manual check", 'black', 
                     ifelse(deployments_ResMed$needs_battery_change == 1, 'orange', 'darkblue')),
  library = "fa",
  squareMarker = F
  # ,spin = T
)

icon_anchor_connectmed <- leaflet::makeAwesomeIcon(
  icon = "anchor",
  iconColor = 'lightgray',
  markerColor =  ifelse(deployments_CONNECTMED$needs_battery_change == "manual check", 'black', 
                     ifelse(deployments_CONNECTMED$needs_battery_change == 1, 'orange', 'darkblue')),
  library = "fa",
  squareMarker = F
  # ,spin = T
)
...
receiver_map <- 
  ...
  #            fillOpacity = 0) %>%
#
    addAwesomeMarkers(data = deployments_ResMed, #%>% 
                        # dplyr::filter(acoustic_project_code == "ResMed"),
                   icon = icon_anchor_resmed,
                   lat = ~deploy_latitude,
 ...
 addAwesomeMarkers(data = deployments_CONNECTMED,
                  icon = icon_anchor_connectmed,
                  lat = ~deploy_latitude,
                  lng = ~deploy_longitude,
                  popup = ~paste(
                          "<b>Station:</b> ", station_name, "<br>",
  ...
  addLegend(position = "topright", opacity = 1,
            colors = deployments_CONNECTMED$colour %>% unique(), labels = c("Needs manual check", "Needs battery change", "Battery is OK")) %>%
  leaflet::addLayersControl(overlayGroups = c("ResMed","CONNECT-MED"),  
...
```{r testtable2, include = FALSE, echo=FALSE}
# for the moment, I only get the datatable() obj to show in Tab 2 & 3 when this code chunk is there...Magic
DT::datatable(all_deployments %>% head())
```
